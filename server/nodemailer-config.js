...
module.exports.sendConfirmationEmail = (name, email, confirmationCode) => {
  ...
  module.exports.sendCompletingMail = (emailB, imieB, nazwiskoB, emailTo, idNot ) => {
    console.log("Check");
    transport.sendMail({
      from: user,
      to: emailTo,
      subject: "Dane pobierajacego ogloszenie ",
      html: `<h1>Dane pobierajacego ogloszenie ${idNot} </h1>
          <p>email: ${emailB} imie:${imieB} nazwisko:${nazwiskoB}</p>
          </div>`,
    }).catch(err => console.log(err));
  };